...
public:
    ...
    void init_w(int num1,int num2,int batch);
...
void node::init_w(int num1,int num2,int batch)
{
    cudaMalloc((void**)&value,sizeof(double)*num1*batch);
    cudaMalloc((void**)&W,sizeof(double)*num1*num2);
}
...
public:
    ...
    //void ht(double *W_u2,double)
};

net::net() {
    //初始化输入与隐藏偏置权重
    srand((unsigned)time(NULL));
    b1= (rand() % 100) / (double)100;
    b2 = (rand() % 100) / (double)100;
    rate = 0.028; //学习率
    O = new double[OPNNUM*BATCHSIZE];
    //初始化输入到隐藏节点个数
    //init matrix W1 IPNNUM*HDNNUM
    
    inlayer.init_w(IPNNUM,K1,BATCHSIZE);
    
    hidlayer_U2.init_w(K1,K2,BATCHSIZE);
    hidlayer_B1.init_w(K2,HDNNUM,BATCHSIZE);
    hidlayer_U1.initNode(HDNNUM,OPNNUM,BATCHSIZE);
    
    /*hidlayer_U2.initNode(K2,K1,BATCHSIZE);
    hidlayer_B1.initNode(K1,HDNNUM,BATCHSIZE);
    hidlayer_U1.initNode(HDNNUM,OPNNUM,BATCHSIZE);*/

    inlayer_tmp.initNode(IPNNUM,HDNNUM,BATCHSIZE);
    ...
    cublasCreate(&handle);
    // tt_random(inlayer_tmp.W, inlayer.W, hidlayer_U2.W, hidlayer_B1.W, HDNNUM, IPNNUM, K1, K2, handle);
    tt(inlayer_tmp.W, inlayer.W, hidlayer_U2.W, hidlayer_B1.W, HDNNUM, IPNNUM, K1, K2, handle);
}
...
void net::backward(double *T) {
    ...
    back_cuda(Tg,outlayer.value,hidlayer_U1.value,hidlayer_U1.W,hidlayer_B1.value,hidlayer_B1.W,
              hidlayer_U2.value,hidlayer_U2.W,inlayer.value,inlayer.W,IPNNUM,K2,K1,HDNNUM,OPNNUM,BATCHSIZE,rate,handle);
}
