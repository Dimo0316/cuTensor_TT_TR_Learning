...
void basicQR(cusolverDnHandle_t cusolverH, dt *d_A, dt *d_R, int m, int n){
    // printf("start qr for m: %d, n:%d\n", m,n);
    dt *d_work = NULL, *d_tau = NULL;
    int *devInfo = NULL;
    int  lwork = 0; 
...
__global__ void Vector2_Multiply_By_Elements (const dt* target, const dt* nn_out, int n, dt* out){
	int tid = blockIdx.x*blockDim.x + threadIdx.x;
	const long temp = blockDim.x*gridDim.x;
	while(tid<n)
	{
		out[tid]=target[tid]-nn_out[tid];
		tid+=temp;
	}
	__syncthreads();
}

__global__ void Vector1_Multiply_By_Elements (const dt* out_layer, dt* dsg, int n){
	int tid = blockIdx.x*blockDim.x + threadIdx.x;
	const long temp = blockDim.x*gridDim.x;
	while(tid<n)
	{	
		if(out_layer[tid] < FLOATZERO)
			dsg[tid] = 0;
		tid+=temp;
	}
	__syncthreads();
...
dt loss_gpu_cross(dt *T, dt *O, int n, int batch, cublasHandle_t handle)
{	
	//A 实际值  B预测值
	// printf("\nT\n");
	// printTensor(T,10,1,1);
	// printf("\nO\n");
	// printTensor(O,10,1,1);
	dim3 block_nbatch((batch*n+1024-1)/1024,1,1);
	dt* Temp= NULL;
	dt result=0.0;
	cudaMalloc((void**)&Temp, sizeof(dt)*n*batch);
	loss_gpu_cross_kernel<<<block_nbatch, 1024>>>(Temp, T, O, n*batch);
	cudaDeviceSynchronize();

	// printf("\nTemp\n");
	// printTensor(Temp,10,1,1);
	cublasSasum(handle, n*batch, Temp, 1, &result);
	
	result = result / (dt)batch;
	if(Temp) cudaFree(Temp);Temp=NULL;
	return result;
}
...
void back_cuda(dt *Y_target,dt *Y_out, dt *W2,
               dt *outG4, dt *G4, dt *outG3, dt *G3,
               dt *outG2, dt *G2, dt *outG1, dt *G1,
               dt *input, 
               int* inp_modes, int* out_modes, int* ranks, int* out_values, int out, int batch, dt rate, dt momentum, cublasHandle_t handle){
	// printf("start back_cuda \n");
	cublasStatus_t cublas_status = CUBLAS_STATUS_SUCCESS;
	int hid = out_modes[0] * out_modes[1] * out_modes[2] * out_modes[3];
	...
	Vector2_Multiply_By_Elements<<<block2,1024>>>(Y_target, Y_out, out*batch, d_thta5);
	dt alpha=1.0, beta=0.0;
...
///TODO /修改
	...
	assert(cublas_status == CUBLAS_STATUS_SUCCESS);
	// printf("G4\n");
	// printTensor(G4,out_modes[3]*ranks[4], ranks[3]*inp_modes[3],1);
	...
	assert(cublas_status == CUBLAS_STATUS_SUCCESS);
	// printf("G3\n");
	// printTensor(G3,out_modes[2]*ranks[3], ranks[2]*inp_modes[2],1);
	...
	assert(cublas_status == CUBLAS_STATUS_SUCCESS);
	// printf("G2\n");
	// printTensor(G2,out_modes[1]*ranks[2], ranks[1]*inp_modes[1],1);
	...
	cudaDeviceSynchronize();
	// printf("G1\n");
	// printTensor(G1,out_modes[0]*ranks[1], ranks[0]*inp_modes[0],1);
