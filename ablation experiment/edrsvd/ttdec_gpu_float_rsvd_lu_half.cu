...
// step 4: compute spectrum
    ...
    cout << "cusolver_status: " << cusolver_status <<endl;
    if (cusolver_status == CUSOLVER_STATUS_INVALID_VALUE)
        cout << "CUSOLVER_STATUS_INVALID_VALUE: " << cusolver_status <<endl;
    else if (cusolver_status == CUSOLVER_STATUS_ALLOC_FAILED)
        cout << "CUSOLVER_STATUS_ALLOC_FAILED: " << cusolver_status <<endl;
    else if (cusolver_status == CUSOLVER_STATUS_EXECUTION_FAILED)
        cout << "CUSOLVER_STATUS_EXECUTION_FAILED: " << cusolver_status <<endl;
    else if (cusolver_status == CUSOLVER_STATUS_INTERNAL_ERROR)
        cout << "CUSOLVER_STATUS_INTERNAL_ERROR: " << cusolver_status <<endl;
    else if (cusolver_status == CUSOLVER_STATUS_MATRIX_TYPE_NOT_SUPPORTED)
        cout << "CUSOLVER_STATUS_MATRIX_TYPE_NOT_SUPPORTED: " << cusolver_status <<endl;
...
///对B进行分解
    ...
    float *d_U2 = NULL, *d_SVT2 = NULL;
    cudaStat2 = cudaMalloc ((void**)&d_U2  , sizeof(float) * m * ks);
    cudaStat1 =cudaMalloc((void**)&d_SVT2, sizeof(float)*ks*n); 
    assert(cudaSuccess == cudaStat1);
    assert(cudaSuccess == cudaStat2);
...
/// U = Q*UT
    ...
    // printMatrix_Device(10, 10, d_U2, 10, "d_U2");
    // printMatrix_Device(10, 10, d_SVT2, 10, "d_SVT2");

    // / cudaStat1 = cudaMemcpy2D(d_U, sizeof(float)*k, d_U2, sizeof(float)*ks, sizeof(float)*k, m, (cudaMemcpyKind)3);
    // cudaStat2 = cudaMemcpy2D(d_SVT, sizeof(float)*n, d_SVT2, sizeof(float)*n, sizeof(float)*n, k, (cudaMemcpyKind)3);
    matrixGetSub_VT_host(d_SVT2, d_SVT, (long)ks, (long)n, (long)k);
    assert(cudaSuccess == cudaStat1);
    assert(cudaSuccess == cudaStat2);
    // printMatrix_Device(10, 10, d_U, 10, "d_U");
    // printMatrix_Device(10, 10, d_SVT, 10, "d_SVT");


    if(d_SVT2) cudaFree(d_SVT2); d_SVT2 = NULL;
...
float ttdec_gpu_float(float* h_tensor, int* h_n_array, int* h_r_array,double *eps, float* singleError, float* singleComRatio ){
    ...
    cudaStat2 = cudaMalloc ((void**)&d_U  , sizeof(float) * m * k);
    assert(cudaSuccess == cudaStat1);
    assert(cudaSuccess == cudaStat2);
    // GPUTimer timer;
    // timer.start();
...
///计算rsvd_LU获得矩阵 [U SVT]
    ...
    // timerSVD2.start();
    dt *d_A2 = NULL;
    cudaStat1 =cudaMalloc((void**)&d_A2, sizeof(dt)*k*n); 
    // int s2 = 8;
...
///根绝截断值将核G1复制到内存 
 ...
 /* step 1: create cusolver handle, bind a stream */
    cudaStat1 = cudaMalloc ((void**)&d_U2   , sizeof(dt)*m2*k); 
