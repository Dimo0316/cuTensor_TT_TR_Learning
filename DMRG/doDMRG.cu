...
t1=clock();	
	...
	{		
		...
		norm_temp = norm(d_sWeight[i],norm_temp,sizeA[i][2]*sizeA[i][2],handle);
...
/*
	dt *temp_L = new dt[sizeL[Nsite-1][0]*sizeL[Nsite-1][1]*sizeL[Nsite-1][2]];
	cudaMemcpy(temp_L,d_L[Nsite-1],sizeof(dt)*sizeL[Nsite-1][0]*sizeL[Nsite-1][1]*sizeL[Nsite-1][2],cudaMemcpyDeviceToHost);
	for(int i = 0; i < sizeL[Nsite-1][0]*sizeL[Nsite-1][1]*sizeL[Nsite-1][2]; ++i) {		
		cout<<temp_L[i]<<" _ ";
	}
	cout<<endl;
*/
...
/*
  		dt *psi_temp =new dt[sizeA[p][0]*sizeA[p][1]*sizeA[p][2]];
  		cudaMemcpy(psi_temp,d_psiGround,sizeof(dt)*sizeA[p][0]*sizeA[p][1]*sizeA[p][2],cudaMemcpyDeviceToHost);
  		cout<<"number site :"<<p<<endl;
  		for(unsigned i = 0; i < sizeA[p][0]*sizeA[p][1]*sizeA[p][2]; ++i) {
  			cout<<psi_temp[i]<<" _ ";
  		}
  		cout<<endl;
*/
		...
		division(d_QRweight[p],norm_temp,d_QRweight[p],sizeA[p][0],sizeA[p][0],1,handle);
  		...
  		if(p != 0){  			
  			...
  			//更新R[p-1]
  			ncon2(d_R[p],d_M,d_A[p],sizeA[p],sizeR[p],d_R[p-1],handle,tensor_handle);//没问题
/*
  			dt *psi_temp =new dt[sizeR[p-1][0]*sizeR[p-1][1]*sizeR[p-1][2]];
  			cudaMemcpy(psi_temp,d_R[p-1],sizeof(dt)*sizeR[p-1][0]*sizeR[p-1][1]*sizeR[p-1][2],cudaMemcpyDeviceToHost);
  			cout<<"ncon2 geng xin R  :"<<p<<endl;
  			for(unsigned i = 0; i < sizeR[p-1][0]*sizeR[p-1][1]*sizeR[p-1][2]; ++i) {
  				cout<<psi_temp[i]<<" _ ";
  			}
  			cout<<endl;
*/
  	...
  	{	
  		...
  		size_psi =sizeA[p][0]*sizeA[p][1]*sizeA[p][2];
/*
  		dt *psi_temp2 =new dt[sizeA[p][0]*sizeA[p][1]*sizeA[p][2]];
  		cudaMemcpy(psi_temp2,d_psiGround,sizeof(dt)*sizeA[p][0]*sizeA[p][1]*sizeA[p][2],cudaMemcpyDeviceToHost);
  		cout<<"number site :"<<p<<endl;
  		for(unsigned i = 0; i < sizeA[p][0]*sizeA[p][1]*sizeA[p][2]; ++i) {
  			cout<<psi_temp2[i]<<" _ ";
  		}
...
*/	
		...
		division(d_QRweight[p+1],norm_temp,d_QRweight[p+1],sizeA[p][2],sizeA[p][2],1,handle);
  		...
  		if(p != Nsite-1)
  		{
  			ncon(d_L[p],d_M,d_A[p],sizeA[p],sizeL[p],d_L[p+1],handle,tensor_handle);//没问题
/*
  		dt *psi_temp2 =new dt[sizeL[p+1][0]*sizeL[p+1][1]*sizeL[p+1][2]];
  		cudaMemcpy(psi_temp2,d_L[p+1],sizeof(dt)*sizeL[p+1][0]*sizeL[p+1][1]*sizeL[p+1][2],cudaMemcpyDeviceToHost);
  		cout<<"L[p+1] de geng xin  :"<<p<<endl;
  		for(unsigned i = 0; i < sizeL[p+1][0]*sizeL[p+1][1]*sizeL[p+1][2]; ++i) {
  			cout<<psi_temp2[i]<<" _ ";
  		}
  		cout<<endl;*/
