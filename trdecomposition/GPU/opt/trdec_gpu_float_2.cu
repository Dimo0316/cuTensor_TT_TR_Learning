...
///获得截断索引 truncat_number 也就是h_r_array
    ...
    h_r_array[0] = econ_s - truncat_number_test + 2;
    // h_r_array[1] = min(econ_s - truncat_number_test + 2, h_r_array[1]);
    int truncat_number =0;
    truncat_number = h_r_array[0];
    if(h_r_array[0] % 2 != 0)
        truncat_number = h_r_array[0] + 2 - h_r_array[0] % 2;
    else
        truncat_number = h_r_array[0];

    cout << "truncat_number1:" << truncat_number << endl;
    int r1 = 2;
    int r2 = truncat_number/r1;
...
///TODO 计算中间变量
    ...
    //TODO计算对角矩阵乘V转置矩阵
    //利用
    dt *d_A2 = NULL, *d_VT = NULL, *d_G1truncat=NULL, *d_A2Mid = NULL;//, *d_Struncat = NULL;//, *d_Vtruncat = NULL;
    cudaStat1 =cudaHostAlloc((void**)&h_G1, sizeof(dt)*ldu*truncat_number, 0);
    cudaStat2 =cudaMalloc((void**)&d_G1truncat, sizeof(dt)*truncat_number*ldu);
    cudaStat3 =cudaMalloc((void**)&d_A2, sizeof(dt)*truncat_number*n); //20*400
    cudaStat4 =cudaMalloc((void**)&d_VT, sizeof(dt)*econ_s*n); //20*400
    cudaStat5 =cudaMalloc((void**)&d_A2Mid,    sizeof(dt)*truncat_number*n);
    assert(cudaStat1 == cudaSuccess);
...
///根绝截断值将核G1复制到内存
    cublas_status = cublasScopy(handle, truncat_number*m,
                            d_U, 1,
                            d_G1truncat, 1);
    assert(cublas_status == CUBLAS_STATUS_SUCCESS);
    cudaStat1 = cudaDeviceSynchronize();
    ...
    dim3 blocksldur1r2((r1*r2*ldu+1024-1)/1024,1,1);
    tensorToMode2<<<blocksldur1r2,threads>>>(d_G1truncat, d_U, ldu, r1, r2);
...
//再计算矩阵  列向量乘法  econ_s*econ_s  econ_s*n
    cublas_status = cublasSdgmm(handle,
                                ...
                                d_VT, econ_s,
                                d_S, 1,
                                d_A2Mid, truncat_number);
    assert(cublas_status == CUBLAS_STATUS_SUCCESS);
    cudaStat1 = cudaDeviceSynchronize();
    assert(cudaStat1 == cudaSuccess);

    dim3 blocksr1r2n((r1*r2*n+1024-1)/1024,1,1);
    GPUTimer timer2;
    timer2.start();
///TODO 将d_AT2 从r1*r2*m 转变成 r2*m*r1
    // tensorToMode2<<<blocksr1r2n,threads>>>(d_A2Mid,d_A2,r1,r2,n);
    // cudaStat1 = cudaDeviceSynchronize();
    // assert(cudaStat1 == cudaSuccess);
    // tensorToMode3<<<blocksr1r2n,threads>>>(d_A2,d_A2Mid,r2,r1,n);
    // cudaStat1 = cudaDeviceSynchronize();
    // assert(cudaStat1 == cudaSuccess);
    // tensorToMode2<<<blocksr1r2n,threads>>>(d_A2Mid,d_A2,n,r2,r1);
    // cudaStat1 = cudaDeviceSynchronize();
    // assert(cudaStat1 == cudaSuccess);

    tensorToMode231<<<blocksr1r2n,threads>>>(d_A2Mid,d_A2,r1, r2, n);
    cudaStat1 = cudaDeviceSynchronize();
    assert(cudaStat1 == cudaSuccess);
...
///TODO第二部分计算
    ...
    const int n2 = h_n_array[2] * h_r_array[0];  // 200
...
///TODO
    ...
    dim3 blocks2((econ_s2+1024-1)/1024,1,1);
    hardm<<<blocks2,threads>>>(d_S2,d_S2,d_Shardm2,econ_s2);
    // printMatrix_Device(econ_s2,1,d_S2,econ_s2,"S2");
    // printMatrix_Device(econ_s,1,d_Shardm,econ_s,"Shardm");
...
///截断V
    ...
    comp_rat_number = comp_rat_number + ldu2*truncat_number + ldv2*truncat_number;
