...
//For test
    ...
    cudaStat5 = cudaMalloc ((void**)&d_G1  , sizeof(float) * truncat_number * m);
    assert(cudaSuccess == cudaStat1);
    assert(cudaSuccess == cudaStat2);
    assert(cudaSuccess == cudaStat3);
...
///TODO 计算中间变量
    //TODO将s向量变成对角矩阵
    //TODO计算对角矩阵乘V转置矩阵
    dt *d_A2 = NULL, *d_VT = NULL, *d_A2Mid=NULL;
    cudaStat1 =cudaMalloc((void**)&d_A2, sizeof(dt)*truncat_number*n); //56*1000*1000
    cudaStat2 =cudaMalloc((void**)&d_A2Mid, sizeof(dt)*truncat_number*n);
    cudaStat3 =cudaMalloc((void**)&d_VT, sizeof(dt)*ks*n);
    assert(cudaStat1 == cudaSuccess);
    assert(cudaStat2 == cudaSuccess);
    assert(cudaStat3 == cudaSuccess);
    cudaDeviceSynchronize();
