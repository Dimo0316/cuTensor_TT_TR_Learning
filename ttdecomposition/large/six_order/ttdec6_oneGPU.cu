...
// /usr/local/cuda/bin/nvcc -lcublas -lcusolver -lcurand -std=c++11 ttdec6_oneGPU.cu -o oneGPU
...
// void largeMatrixSelfMulti_once_device_float(cublasHandle_t cublasH, dt *d_A,const int &m,const long long &n, dt *d_AAT){
...
//      cublas_status = cublasSgemm(cublasH, CUBLAS_OP_N, CUBLAS_OP_T,
//                            m, m, n,
//                            &alpha, d_A, m,
//                            d_A, m,
//                            &beta0, d_AAT, m);
...
float ttdec_half_lu_host_6(dt* h_A, int *ttRanks, const int *ttDemns, const int &calculateTimes, const int &p, const bool calError){
    ...
    //1
        ...
        assert(cudaStat2 == cudaSuccess);
        // printMatrix_Device(m, n, d_A, m, "A");
        longMatrixSVD_Eign_once_host(cublasH, cusolverH, h_A, m, n, ttRanks[1] , h_G1, d_A2, p);
