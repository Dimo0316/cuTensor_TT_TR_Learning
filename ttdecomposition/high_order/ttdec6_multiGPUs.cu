...
// /usr/local/cuda/bin/nvcc -Xcompiler -fopenmp -lcublas -lcusolver -lcurand -std=c++11 ttdec6_multiGPUs.cu -o multiGPUs
...
void largeMatrixSelfMulti_slice_host_multiGPU(cublasHandle_t cublasH, dt *h_A, const int &m, const long long &n, dt *d_AAT, const int &p){
...
// void smallargeMatrixMulti_slice_host_Tans_multiGPU(cublasHandle_t cublasH1, dt *d_G, dt *h_A, const int &ttRank, const int &m, const int &n, dt *d_A2, const int &p){
...
//         {      
//             cudaSetDevice(1);
//             smallargeMatrixMulti_slice_host_Tans(cublasH1, d_G11, h_A+m*n_half, ttRanks[1], m, n_half, d_A21, p);
//             // if(m>24 && n>pow(m,4)){
//             //     // printf("smallargeMatrixMulti_slice_device\n");
//             //     smallargeMatrixMulti_slice_device(cublasH1, d_G11, d_A1, ttRanks[1], m, n_half, d_A21, p);
//             //     // printMatrix_Device(10, 10, d_A2, 10, "A2");
...
// void longMatrixSVD_Eign_once_host_multiGPU(cublasHandle_t cublasH, cusolverDnHandle_t cusolverH, dt *h_A, const int &m, const long long &n, const int &ttRank, dt *h_G, dt *d_A2, const int &p){
...
//     largeMatrixSelfMulti_slice_host_multiGPU(cublasH, h_A, m, n, d_AAT, p);
//     // printMatrix_Device( m, m, d_AAT, m, "AAT");
//     // printf("basicEig\n");
//     basicEig(cusolverH, d_AAT, m, d_W);
//      // printMatrix_Device( m, m, d_AAT, m, "AAT");
...
//     smallargeMatrixMulti_slice_host_Tans_multiGPU(cublasH, d_G, h_A, ttRank, m, n, d_A2, p);
//     // printMatrix_Device(10, 10, d_A2, 10, "A2");
...
float ttdec_half_lu_host_6_multiGPU(dt* h_A, const int *ttRanks, const int *ttDemns, const int &calculateTimes, const int &p, bool calError){
    ...
    //1
        ...
        // assert(cudaStat2 == cudaSuccess);
        // // printMatrix_Device(m, n, d_A, m, "A");
        // longMatrixSVD_Eign_once_host_multiGPU(cublasH, cusolverH, h_A, m, n, ttRanks[1] , h_G1, d_A2, p);
