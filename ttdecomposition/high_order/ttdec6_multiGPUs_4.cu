...
// /xfs/home/tensor_zy/anaconda3/envs/ht2/bin/nvcc -Xcompiler -fopenmp -lcublas -lcusolver -lcurand -std=c++11 ttdec6_multiGPUs_4.cu -o multiGPUs4
...
double calMSE(cublasHandle_t cublasH, dt *h_A, dt *h_G1, dt *h_G2, dt *h_G3, dt *h_G4, dt *h_G5, dt *h_G6, const int *ttRanks, const int *ttDemns, const int p){
    ...
    // printMatrix(2,2,h_G1,2,"h_G1");
    // printMatrix(2,2,h_G2,2,"h_G2");
    // printMatrix(2,2,h_G3,2,"h_G3");
    // printMatrix(2,2,h_G4,2,"h_G4");
    // printMatrix(2,2,h_G5,2,"h_G5");
    // printMatrix(2,2,h_G6,2,"h_G6");
...
void largeMatrixSelfMulti_slice_host(cublasHandle_t cublasH, dt *h_A, const long long &over, const long long &m, const long long &n, dt *d_AAT, const int &p, const int & streamNumber){
    ...
    for(long long i=0;i<p;i++){
        ...
        // if(cudaStat3 != 0){
        //     printMatrix(m, 10, h_A+m*slice*i, m, "d_hA");
        //     printMatrix_Device(m, 10, d_tempA, m, "d_tempA");
        // }
        // printf("cudaStat3: %d and i= %d, and threads:%d \n", cudaStat3, i, omp_get_thread_num());
        // cudaStat4 = cudaDeviceSynchronize();
...
#pragma omp parallel num_threads(4)
        ...
        #pragma omp parallel num_threads(4)
        {
            ...
            {      
                cudaSetDevice(device[3]);
                smallargeMatrixMulti_slice_host_Tans(cublasH1, d_G13, h_A+((long long)m)*n_half*(long long)3, ttRanks[1], m, n_half, d_A23, p);
                printf("thread 3 finish0\n");
            }
            else if(cpuid == 2)
            {      
                cudaSetDevice(device[2]);
                smallargeMatrixMulti_slice_host_Tans(cublasH1, d_G12, h_A+((long long)m)*n_half*(long long)2, ttRanks[1], m, n_half, d_A22, p);
...
int main(){
    ofstream ofs;
    ofs.open("result_4.txt", ios::out);
    for(int i = 8; i < 68; i=i+4){
        const int k = i;
