...
void ttdec_gpu_float(float* h_tensor, int* h_n_array, int* h_r_array,double *eps, float* singleError, float* singleComRatio ){
...
///根绝截断值将核G1复制到内存
    ...
    // printMatrix_Device(h_n_array[0],h_r_array[1],d_U,h_n_array[0],"d_U1");
...
///截断V
    ...
    // cudaDeviceSynchronize();
    // cudaStat1 = cudaMemcpy(d_A, h_tensor, sizeof(dt)*calAllNumber, cudaMemcpyHostToDevice);
    ...
    // singleError[0] = (float)(re/before);
    // singleComRatio[0] = comp_ratio;
